...
public class XBee implements IXBee {
	...
	public XBee() {
		this(new XBeeConfiguration().withMaxQueueSize(100).withStartupChecks(true).withConnectionProvider(new SerialPortConnectionProvider()));
	}
	
	public XBee(XBeeConfiguration conf) {
		this.conf = conf;
		this.connectionProvider = conf.getConnectionProvider();
	...
	private void doStartupChecks() throws XBeeException {
		...
		try {
			...
			info = new XBeeInformation(radioType, vr.getValue(), id.getValue(), sh.getValue(), sl.getValue());
	...
	public void open(String port, int baudRate) throws XBeeException {
		try {
			...
			this.initConnection(connectionProvider.open(port, baudRate));
